...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['profileImage']) && $_FILES['profileImage']['error'] == 0) {
        ...
        $sql = "UPDATE employer SET profileImage = ?, company_name = ?, company_address = ?, email = ?, phone = ? WHERE username = ?";
        ...
        // Prepare SQL statement to insert into database
        // Use prepared statements to avoid SQL injection
        $sql = "UPDATE employer SET company_name = ?, company_address = ?, email = ?, phone = ? WHERE username = ?";
        $stmt = $conn->prepare($sql);
        if ($stmt === false) {
            die('Error preparing statement: ' . htmlspecialchars($conn->error));
        }
